...
# ---- Authorization (local allowlist; FAILS CLOSED) ----

class AuthorizationDeniedError(Exception):
    """The IDP authenticated the user, but local policy refuses them.

    Distinct from every transport/credential error: authentication SUCCEEDED and the ID token
    verified — we decline to mint a Session. Callers translate it per-path so a denied user is
    bounced to re-login rather than shown a 503 "IDP unreachable".
    """


# Explicit opt-out. Because an unset/empty allowlist DENIES, "let every authenticated identity
# in" must be stated on purpose. A single "*" is the whole allowlist; never combined with names.
_ALLOW_ALL_SENTINEL = "*"


def _parse_allowed_users(raw: Any) -> tuple[str, ...]:
    """Normalise the allowlist (config.yaml list, or comma/whitespace string from env) to a
    lowercase tuple. Empty result = no allowlist = authorize NOBODY (see _authorize_claims)."""
    if raw is None:
        return ()
...
class SelfHostedOIDCProvider(JwtOAuthProvider):
    ...
    def __init__(self, *, issuer: str, client_id: str, scopes: str = _DEFAULT_SCOPES, client_secret: str = "",
                 allowed_users: Any = None) -> None:
        if not issuer:
    ...
    def _grant(
        ...
        try:
            ...
            # Authenticated but declined by local policy. Translate to the caller's bad-request
            # exception so middleware forces re-login rather than treating it as a transient IDP
            # outage (503 + retry, which would loop). On refresh that's RefreshExpiredError, which
            # correctly clears the session of a user removed from the allowlist mid-session.
            raise bad_request_exc(str(exc)) from exc

    def verify_session(self, *, access_token: str) -> Optional[Session]:
        """As the base, but a denied identity yields None, not an exception: ``verify_session``'s
        contract is None for a token it won't honour (a ProviderError would mean "IDP unreachable"
        → a 503 the user could never clear). None sends middleware down the refresh path, which
        also denies, landing the user at /login — this is what revokes access on the NEXT REQUEST
        for a user removed from the allowlist mid-session."""
        try:
    ...
    def _authorize_claims(self, claims: Dict[str, Any]) -> None:
        """Enforce the local allowlist. FAILS CLOSED: an unset/empty allowlist authorizes NOBODY,
        because on a shared IDP (one realm, many dashboards) authentication alone would let any
        realm user in, and a missing env var / typo'd key would grant everyone access invisibly.

        Called from ``_session`` — the chokepoint every Session flows through (login, refresh, and
        per-request ``verify_session``) — so removing a user takes effect on their next request.
        Matching is case-insensitive on ``preferred_username`` / ``email`` (plus the email
        local-part). Only claims from an already signature/iss/aud-verified ID token reach here.
        """
        if _ALLOW_ALL_SENTINEL in self._allowed_users:  # deliberate opt-out
            return
        if not self._allowed_users:
            logger.error(
                "%s: denying authenticated identity sub=%s — no allowed_users configured, so "
                "NOBODY is authorized. Set dashboard.oauth.self_hosted.allowed_users (or "
                "HERMES_DASHBOARD_OIDC_ALLOWED_USERS) to the permitted identities, or to %r to "
                "allow every authenticated user.", _TAG, claims.get("sub", "<unknown>"), _ALLOW_ALL_SENTINEL)
            raise AuthorizationDeniedError(
                "no allowed_users configured — this dashboard authorizes nobody until an allowlist is set")
        candidates = set()
        ...
        if candidates & set(self._allowed_users):
            return
        # Log the subject only, never the whole claim set (PII); operator correlates via the IDP.
        logger.warning("%s: denying authenticated identity sub=%s — not in allowed_users allowlist (%d entries)",
                       _TAG, claims.get("sub", "<unknown>"), len(self._allowed_users))
        raise AuthorizationDeniedError("authenticated identity is not permitted on this dashboard")
...
def _settings() -> dict:
    ...
    # Authorization allowlist. config.yaml (a YAML list) is canonical; the env var is a bridge for
    # orchestrators injecting per-pod identity. Unset/empty ⇒ nobody authorized (fail closed).
    allowed_users_raw: Any = os.environ.get("HERMES_DASHBOARD_OIDC_ALLOWED_USERS", "").strip()
...
def register(ctx) -> None:
    ...
    if kw is not None:
        allowed = kw.get("allowed_users") or ()
        if _ALLOW_ALL_SENTINEL in allowed:
            authz_desc = "DISABLED (all authenticated users allowed)"
        elif allowed:
            authz_desc = f"{len(allowed)} entries"
        else:
            authz_desc = "NOBODY (fail-closed)"
            logger.warning(
                "%s: no allowed_users configured — this dashboard authorizes NOBODY. Set "
                "dashboard.oauth.self_hosted.allowed_users (or HERMES_DASHBOARD_OIDC_ALLOWED_USERS), "
                "or %r to allow everyone.", _TAG, _ALLOW_ALL_SENTINEL)
        logger.info(
            "dashboard-auth-self-hosted: registered provider (issuer=%s, client_id=%s, scopes=%r, "
            "confidential=%s, authorized=%s)",
            kw["issuer"], kw["client_id"], kw["scopes"], bool(kw["client_secret"]),
            authz_desc)  # never log the secret itself
...
import os  # noqa: F401,F811,E402  (F811: also imported at top level for _settings; re-exported here for external plugins)

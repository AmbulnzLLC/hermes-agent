...
# TEAMS_SDK_AVAILABLE True from find_spec means "installed", NOT "imported" — ``App is not None``
...
# Fork additions: FileConsent (outbound files) + clarify-card choice inputs.
FileConsentInvokeActivity = FileUploadInfo = Choice = ChoiceSetInput = None  # type: ignore[assignment,misc]

from gateway.config import Platform, PlatformConfig
from gateway.platforms.helpers import MessageDeduplicator
from gateway.platforms.base import (
    gateway_trust_env, BasePlatformAdapter, ExecApprovalPrompt, SendResult, SUPPORTED_DOCUMENT_TYPES,
    cache_audio_from_url, cache_image_from_url, cache_media_bytes_async, cache_video_from_url,
)
...
# ``None`` host → aiohttp binds IPv4 + IPv6 ("0.0.0.0" bound IPv4 ONLY and was unreachable on
# IPv6-only private networks such as Fly.io 6PN — same bug as the LINE adapter (NS-603) and
# gateway/platforms/webhook.py (d542894ad)). Pin via TEAMS_HOST or extra.host.
_DEFAULT_HOST = None
...
# Bot Framework attachment endpoints needing a Bearer token: use the shared allowlist predicate
# (_is_botframework_attachment_url), which is the stronger guard — it also rejects a non-443 port,
# so a `https://smba.trafficmanager.net:444/...` lookalike can't harvest the bot token, and it
# covers the US-gov host. The fork's separate _is_bf_attachment_url/_BF_ATTACHMENT_HOSTS pair
# checked the same host set without the port check and has been collapsed into it.
_is_bf_attachment_url = _is_botframework_attachment_url
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Falsy host (unset/"") collapses to the dual-stack default (None), so aiohttp binds both
        # IPv4 and IPv6. Pin explicitly via extra.host or TEAMS_HOST for a single family.
        _raw_host = extra.get("host") or _get_scoped_secret("TEAMS_HOST", "") or _DEFAULT_HOST
        self._host: Optional[str] = str(_raw_host) if _raw_host else None
        self._app: Optional["App"] = None
        # (token, expiry monotonic ts) for connector attachment auth; refreshed under a lazily
        # created lock (asyncio.Lock() here could bind the wrong loop).
        self._bf_token_cache: Optional[tuple] = None
        self._bf_token_lock: Optional[asyncio.Lock] = None
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # host=None → dual-stack bind (IPv4 + IPv6); see the _DEFAULT_HOST comment.
    ...
    async def _get_botframework_token(self) -> str:
        """Bot Framework bearer token (client credentials), cached until ~5 min before expiry;
        connector attachments are NOT pre-authenticated, unlike SharePoint downloadUrls. The lock is
        created lazily because ``asyncio.Lock()`` in __init__ may bind the wrong loop."""
        import httpx
        if getattr(self, "_bf_token_lock", None) is None:
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 60.0) -> bytes:
        ...
        ``_fetch_attachment_bytes_or_none``, the non-raising wrapper the
        inbound loop uses for Bot Framework ``/v3/attachments/`` endpoints).
    ...
    async def _fetch_attachment_bytes_or_none(self, url: str) -> Optional[bytes]:
        """``_fetch_attachment_bytes`` but ``None`` instead of raising, so the inbound attachment
        loop can fall back to the Graph hosted-content path.

        Bot Framework endpoints (``smba.trafficmanager.net/.../v3/attachments/.../views/original``)
        401 on anonymous GETs; ``_fetch_attachment_bytes`` adds the bearer for BF hosts via
        ``_get_botframework_token``. The fork previously used a separate raw-aiohttp helper with the
        SDK's ``_get_bot_token``; upstream's path is strictly safer (SSRF guard + inbound media size
        cap, so a lying Content-Length can't OOM the gateway), so the two have been collapsed.
        """
        if not url:
            return None
        try:
            return await self._fetch_attachment_bytes(url)
        except Exception as exc:  # noqa: BLE001 — caller falls back to the Graph hosted-content path
            logger.warning("[teams][attach] BF GET %s failed: %s", url, exc)
            return None
            ...
            try:
                ...
                if content_url and content_type.startswith("image/"):
                    if _is_bf_attachment_url(content_url):
                        ...
                        data = await self._fetch_attachment_bytes_or_none(content_url)
                        cached = None
                        if data is not None:
                            sub = content_type.split("/", 1)[1].split(";")[0].strip()
                            ext = _resolve_media_ext(sub, data, "image")
                            try:
                                # Unified cacher (async: the sync variant blocks the gateway loop).
                                # It classifies by filename/MIME rather than trusting a guessed ext.
                                cached_media = await cache_media_bytes_async(
                                    data, filename=att_name or f"image{ext}", mime_type=content_type)
                                cached = cached_media.path if cached_media else None
                                if cached is None:
                                    # Bytes that aren't really an image (e.g. an HTML error page from
                                    # a stale BF URL) — the cacher declines them. Never a silent drop.
                                    logger.warning(
                                        "[teams][attach][%d] failed image validation, not cached "
                                        "(content_type=%r, %d bytes)", idx, content_type, len(data))
                            except Exception as exc:  # noqa: BLE001
                                logger.warning(
                                    "[teams][attach][%d] failed image validation, not cached: %s",
                                    idx, exc,
                                )
                                cached = None
                            logger.info("[teams][attach][%d] cache_media_bytes_async(image) -> %r", idx, cached)
                        if cached is None:
                ...
                if content_type == "application/vnd.microsoft.teams.file.download.info":
                    ...
                    if file_type in _IMAGE_EXTS:
                        ...
                        # Upstream's unified caching entry point (async: the sync variant blocks the
                        # gateway loop — upstream's inbound media-cache offload sweep). Classifies by
                        # filename/MIME, routes to the right cache_* helper, and returns a CachedMedia
                        # we map back onto the fork's (media_urls, media_types, msg_type_override).
                        cached_media = await cache_media_bytes_async(
                        ...
                        logger.info("[teams][attach][%d] dispatch=unknown_bytes -> cache_media_bytes_async ext=%s", idx, file_type)
                        cached_media = await cache_media_bytes_async(data, filename=filename, mime_type="")
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        with suppress(Exception):
            # Prefer the stored ConversationReference (captured from the inbound activity) over
            # App.send's synthesized one: App.send builds a bare ref from just the conversation id
            # + bot account, with no conversation_type/tenant — enough to route text, but a typing
            # activity needs the authoritative context to render at all. Without this, a typing
            # indicator resumed proactively after a clarify wait (background _keep_typing loop)
            # never showed. Mirrors the conv_ref-first dispatch send_card/image/document use.
            conv_ref = self._conv_refs.get(chat_id)
            if conv_ref:

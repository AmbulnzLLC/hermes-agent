...
class TeamsAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._sharepoint_site_id: str = (
            self._extra.get("sharepoint_site_id")
            or _get_scoped_secret("TEAMS_SHAREPOINT_SITE_ID", "")
        )
        self._sharepoint_folder: str = (
            self._extra.get("sharepoint_folder")
            or _get_scoped_secret("TEAMS_SHAREPOINT_FOLDER", "hermes")
        )
    ...
    def _authorize_card_clicker(
        ...
        """``(allowed, denial_text)`` for approval AND clarify card clicks (see ``_card_action_denied``)."""
        denied = self._card_action_denied(ctx.activity.from_)
        return (denied is None, denied or "")

...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "bedrock":
        ...
        # The AnthropicBedrock SDK takes no guardrailConfig (Converse-API-only), so the
        # transport resends this as X-Amzn-Bedrock-Guardrail* headers — load-bearing for
        # IAM policies keyed on bedrock:GuardrailIdentifier, which 403 without them.
        _load_bedrock_guardrail_config(agent)
...
def _load_bedrock_guardrail_config(agent) -> None:
    """Read bedrock.guardrail.* from config.yaml onto agent._bedrock_guardrail_config."""

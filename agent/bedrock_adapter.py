...
def bedrock_guardrail_config(config: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
    ...
    for src, dst in (("stream_processing_mode", "streamProcessingMode"), ("trace", "trace")):
        if gr.get(src):
            out[dst] = gr[src]
    # Bedrock rejects lowercase trace values (accepted: ENABLED, ENABLED_FULL, DISABLED), so a
    # YAML-idiomatic `trace: disabled` would 400 without this normalization (fork).
    if out.get("trace"):
        out["trace"] = str(out["trace"]).upper()
    return out
...
def _tool_call_ns(tool_use_id: str, name: str, input_dict) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        # ensure_ascii=False: Bedrock returns toolUse.input as a native dict, and re-encoding it
        # with the default escapes em dash to a literal \u2014 on Teams clarify cards (fork #16).
        function=SimpleNamespace(name=name, arguments=json.dumps(input_dict, ensure_ascii=False)),
    )

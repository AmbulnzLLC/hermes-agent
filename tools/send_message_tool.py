...
_MEDIA_PLATFORMS_NOTE = "telegram, discord, matrix, weixin, signal, yuanbao, feishu, whatsapp, slack and teams"

# Platforms whose LIVE gateway adapter implements the native send_image_file/send_video/
# send_voice/send_document APIs, so _send_via_adapter delivers media without a standalone
# sender. Media for these must not be dropped with a warning on the generic path.
_ADAPTER_NATIVE_MEDIA = {"buzz", "teams"}
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None,
    ...
    # Generic path: text only, EXCEPT platforms whose live adapter delivers media natively via
    # _send_via_adapter -> _send_live_adapter_media (buzz upstream; teams in this fork — its
    # FileConsent/SharePoint upload needs the running adapter, so it has no standalone sender).
    warning = None
    if media_files and platform_name not in _ADAPTER_NATIVE_MEDIA:

    ...
    def extract_images(content: str) -> Tuple[List[Tuple[str, str]], str]:
        ...
        images = [(m.group(2), m.group(1)) for m in re.finditer(md_pattern, content)
                  if any(m.group(2).lower().endswith(ext) or ext in m.group(2).lower()
                         for ext in markers)]
        # Same marker allowlist on the HTML branch: without it a quoted/example <img> the bot
        # merely mentions (e.g. an inbound Teams AMS URL) boomerangs as a broken attachment.
        images.extend((m.group(1), "") for m in re.finditer(html_pattern, content)
                      if any(m.group(1).lower().endswith(ext) or ext in m.group(1).lower()
                             for ext in markers))
        if not images:

...
# --- SOUL.md immutability (AmbulnzLLC fork) ---
# The fork treats $HERMES_HOME/SOUL.md as immutable agent identity: it must
# match the baked-in docker/SOUL.md exactly on every boot, and the running
# agent (hermes uid) must NOT be able to mutate it. Upstream's seed_one only
# copies on first boot — that's not enough for our contract. We re-copy
# unconditionally as root and chmod 0444 root-owned so even a compromised
# agent process can't rewrite its own soul.
if [ -f "$INSTALL_DIR/docker/SOUL.md" ]; then
    cp -f "$INSTALL_DIR/docker/SOUL.md" "$HERMES_HOME/SOUL.md"
    chown root:root "$HERMES_HOME/SOUL.md" 2>/dev/null || true
    chmod 0444 "$HERMES_HOME/SOUL.md" 2>/dev/null || true
fi

# --- Bedrock guardrail seed (AmbulnzLLC fork) ---
# Writes BEDROCK_GUARDRAIL_ID/VERSION/TRACE from env into config.yaml under
# bedrock.guardrail. No-op when the env vars are unset, so generic deploys
# aren't affected. Must run after seed_one "config.yaml" so the file exists.
s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" \
    "$INSTALL_DIR/docker/seed_admin_config.py" \
    || echo "[stage2] Warning: seed_admin_config.py failed; continuing"
...
# --- GitHub App PEM install (AmbulnzLLC fork) ---
# Reads GITHUB_APP_PEM_SECRET_ID from env, fetches the named AWS Secrets
# Manager secret, and writes the PEM to /opt/data/secrets/github-app.pem
# as hermes:hermes mode 0400. Runs as root because it manages its own
# ownership/perms via os.chown/os.chmod. Silent no-op when the env var is
# unset, crash-loops the pod when set-but-failed (loud > silent for
# pilot deploys whose GitHub auth must work).
"$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/docker/install_github_app_pem.py"

# --- Default taps seed (AmbulnzLLC fork) ---
# Reads HERMES_DEFAULT_TAPS from env and registers each tap in
# $HERMES_HOME/skills/.hub/taps.json. Must run BEFORE skills_sync.py and
# seed_skills.py so tap-relative skill identifiers resolve. Idempotent;
# silent no-op when env unset.
s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" \
    "$INSTALL_DIR/docker/seed_taps.py" \
    || echo "[stage2] Warning: seed_taps.py failed; continuing"
...
# --- Default skills seed (AmbulnzLLC fork) ---
# Reads HERMES_DEFAULT_SKILLS from env and installs each via the same code
# path as `hermes skills install` (quarantine, scan, install, lockfile).
# Must run AFTER skills_sync.py (so the bundled skills tree exists) and
# AFTER seed_taps.py + install_github_app_pem.py (so tap-relative
# identifiers resolve and private-repo network auth works). Silent no-op
# when env unset.
s6-setuidgid hermes "$INSTALL_DIR/.venv/bin/python" \
    "$INSTALL_DIR/docker/seed_skills.py" \
    || echo "[stage2] Warning: seed_skills.py failed; continuing"
